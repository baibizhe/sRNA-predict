...
def Scanpy_IO(file_path:str, test_no_valid:bool = False, batchSize:int = 128, workers:int = 12, log:bool = False, 
    ...
    train_adata = anndata.AnnData(adata[0:slices, :].X, obs=adata[0:slices, :].obs, var=adata.var)

    # validation or test set
    valid_adata = anndata.AnnData(adata[0:slices, :].X, obs=adata[0:slices, :].obs, var=adata.var)


    # turn the cluster numbers into labels
    print("==> Using cluster info for generating train and validation labels")
    qMeta = pd.read_csv("E:/PycharmProjects/cell/data/train.anno.csv")
