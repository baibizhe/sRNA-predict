...
donor_ids = c('Hua2020 Donor1', 'Hua2020 Donor2', 'N6', 'N9', 'N8', 'N11', 'N7', 'N10', 'N3', 'N2', 'N5', 'N4', 'N12', 'N1')
```
...
plots <-c()
for(donor_idx in 1:length(donor_ids)) {
  donor = donor_ids[donor_idx]
  ...
  p_umap2 <-DimPlot(seuratObject, reduction = "umap",label = TRUE, pt.size=1)+ggtitle(donor)
  ...
  plot <- DimPlot(seuratObject, reduction = "umap",label = TRUE, pt.size = 0.1) 
  ...
  plots[donor_idx]<-p_umap2
  print(p_umap2)
  #write.csv(df_hca, file = paste0("expOutput/Rresult/",donor,"level4testb.hcad.csv"),sep = "\t",quote = F, row.names = F, col.names = T)
